...
def service(req_memory, req_core, req_time):
    ...
    if (available_memory >= total_memory*threshold) and \
     (available_core >= total_core*threshold) and \
     ((available_memory-_service_required_memory) > 0) and \
     ((available_core-_service_required_core) > 0):
        ...
        available_memory = available_memory + _service_required_memory
        available_core = available_core + _service_required_core
        return "Task has been executed properly."
    else:
        return "Task execution was failed"
...
if __name__ == '__main__':
    ...
    estimated_time_to_be_available_again = 0
    app.run(port=54323, debug= False, threaded=True)
